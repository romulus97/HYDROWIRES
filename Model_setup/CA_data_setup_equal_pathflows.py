...
def setup(year,hist,hist_year,operating_horizon,perfect_foresight):
    ...
    dispatch_file='../UCED/CA_dispatch_equal_pathflows.py'
    dispatchLP_file='../UCED/CA_dispatchLP_equal_pathflows.py'
    wrapper_file='../UCED/CA_wrapper_equal_pathflows.py'
    simulation_file='../UCED/CA_simulation_equal_pathflows.py'
    price_cal_file='../UCED/CA_price_calculation.py'
    emission_gen_file = '../UCED/CA_emissions_generator.csv'
    emission_calc_file = '../UCED/CA_emission_calculation.py'
    ...
    # create parameter matrix for generators
        ...
        #system wide (daily)
        f.write('param:' + '\t' + 'SimPath66_imports' + '\t' + 'SimPath46_SCE_imports' + '\t' + 'SimPath61_imports' + '\t' + 'SimPath42_imports' + '\t' + 'SimPath24_imports' + '\t' + 'SimPath45_imports:=' + '\n')
        for d in range(0,len(df_imports)):
                f.write(str(d+1) + '\t' + str(df_imports.loc[d,'Path66']) + '\t' + str(df_imports.loc[d,'Path46_SCE']) + '\t' + str(df_imports.loc[d,'Path61']) + '\t' + str(df_imports.loc[d,'Path42']) + '\t' + str(df_imports.loc[d,'Path24']) + '\t' + str(df_imports.loc[d,'Path45']) + '\n')
        f.write(';\n\n')
    
        if perfect_foresight > 0:
            #system wide (daily)
            f.write('param:' + '\t' + 'SimPGE_valley_hydro' + '\t' + 'SimSCE_hydro:=' + '\n')
            for d in range(0,len(df_hydro_PGE)):
                if d <= len(df_hydro_PGE) - len(forecast_days):
                    for fd in forecast_days:
                        fd_index = forecast_days.index(fd)
                        f.write(fd + '\t' + str(d+1) + '\t' + str(df_hydro_PGE.loc[d+fd_index,'fd1']) + '\t' + str(df_hydro_SCE.loc[d+fd_index,'fd1']) + '\n')
         
                else:
                    diff = len(df_hydro_PGE) - d 
                    for fd in forecast_days:
                        fd_index = forecast_days.index(fd)
                        if fd_index < diff:
                            f.write(fd + '\t' + str(d+1) + '\t' + str(df_hydro_PGE.loc[d+fd_index,'fd1']) + '\t' + str(df_hydro_SCE.loc[d+fd_index,'fd1']) + '\n')
                        else:
                            f.write(fd + '\t' + str(d+1) + '\t' + str(df_hydro_PGE.loc[d,fd]) + '\t' + str(df_hydro_SCE.loc[d,fd]) + '\n')
        ...
        #system wide (hourly)
        f.write('param:' + '\t' + 'SimSCE_hydro_minflow' + '\t' + 'SimPGE_valley_hydro_minflow:=' + '\n')
            ...
            for d in range(0,len(df_hydro_PGE)):
                if d <= len(df_hydro_PGE) - len(forecast_days):
                    for fd in forecast_days:
                        fd_index = forecast_days.index(fd) 
                        for h in range(0,24):                
                            f.write(fd + '\t' + str(d*24+fd_index*24+h+1) + '\t' + str(df_SCE_hydro_mins.loc[(d+fd_index)*24+h,'fd1']) + '\t' + str(df_PGE_hydro_mins.loc[(d+fd_index)*24+h,'fd1']) + '\n')
               
                else:
                    diff = d - len(df_hydro_PGE)
                    for fd in forecast_days:
                        fd_index = forecast_days.index(fd) 
                        if fd_index < diff:
                            for h in range(0,24):                
                            ...
                            for h in range(0,24):                
                                f.write(fd + '\t' + str(d*24+fd_index*24+h+1) + '\t' + str(df_SCE_hydro_mins.loc[(d)*24+h,fd]) + '\t' + str(df_PGE_hydro_mins.loc[(d)*24+h,fd]) + '\n')
      ...
      #system wide (hourly)
        f.write('param:' + '\t' + 'SimPath66_exports' + '\t' + 'SimPath42_exports' + '\t' + 'SimPath24_exports' + '\t' + 'SimPath45_exports' + '\t' + 'SimReserves' + '\t' + 'SimPath61_imports_minflow' + '\t' + 'SimPath66_imports_minflow' + '\t' + 'SimPath46_SCE_imports_minflow' + '\t' + 'SimPath42_imports_minflow:=' + '\n')
        for h in range(0,len(df_load)):
                f.write(str(h+1) + '\t' + str(df_exports.loc[h,'Path66']) + '\t' + str(df_exports.loc[h,'Path42']) + '\t' + str(df_exports.loc[h,'Path24']) + '\t' + str(df_exports.loc[h,'Path45']) + '\t' + str(df_reserves.loc[h,'reserves'])  + '\t' + str(df_CA_import_mins.loc[h,'Path61']) + '\t' + str(df_CA_import_mins.loc[h,'Path66']) + '\t' + str(df_CA_import_mins.loc[h,'Path46_SCE']) + '\t' + str(df_CA_import_mins.loc[h,'Path42']) + '\n')
        f.write(';\n\n')
