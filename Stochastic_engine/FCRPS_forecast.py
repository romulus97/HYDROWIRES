...
def simulate(sim_years):
    ...
    spokane_flow = np.zeros((sim_days,f_horizon))
    storage = np.zeros((sim_days,47,f_horizon))
    discharge = np.zeros((sim_days,47,f_horizon))
    powerflow = np.zeros((sim_days,47,f_horizon))
    spill = np.zeros((sim_days,47,f_horizon))
    generation = np.zeros((sim_days,48,f_horizon))
    Spillheads = np.zeros((sim_days,11,f_horizon))
    heads = np.zeros((sim_days,10,f_horizon))
    inflow = np.zeros((sim_days,f_horizon))
    stor = np.zeros((sim_days,f_horizon))
    Storage_left = np.zeros((sim_days,f_horizon))
    ...
    #sfore arrays#
    #########
    ALB_sfore = np.zeros((sim_days,f_horizon))
    ARD_sfore = np.zeros((sim_days,f_horizon))
    BRN_sfore = np.zeros((sim_days,f_horizon))
    DNC_sfore = np.zeros((sim_days,f_horizon))
    DWR_sfore = np.zeros((sim_days,f_horizon))
    GCL_sfore = np.zeros((sim_days,f_horizon))
    HHO_sfore = np.zeros((sim_days,f_horizon))
    LIB_sfore = np.zeros((sim_days,f_horizon))
    MCD_sfore = np.zeros((sim_days,f_horizon))
    
    ###########
    CHJ_head = np.zeros((sim_days,f_horizon))
    DWR_head = np.zeros((sim_days,f_horizon))
    GCL_head = np.zeros((sim_days,f_horizon))
    HHO_head = np.zeros((sim_days,f_horizon))
    IHR_head = np.zeros((sim_days,f_horizon))
    JDA_head = np.zeros((sim_days,f_horizon))
    LGO_head = np.zeros((sim_days,f_horizon))
    LIB_head = np.zeros((sim_days,f_horizon))
    LIM_head = np.zeros((sim_days,f_horizon))
    LWG_head = np.zeros((sim_days,f_horizon))
    ...
    VAQR_LIB = np.zeros((sim_days,f_horizon))
    VAQRnew_LIB = np.zeros((sim_days,f_horizon))
    VAQR_HHO = np.zeros((sim_days,f_horizon))
    VAQRnew_HHO = np.zeros((sim_days,f_horizon))
    FCDend_LIB = np.zeros((no_years,1))
    
    #storage updater
    def storage_update(i,fd,n,previous,s):
    ...
    #for i in range(0,sim_days):
        ...
        for fd in range(0,f_horizon):
            ...
            #####################################3
            #Initialize storage variables based on forecast
            s_1 = storage_update(i,fd,0,storage,s_1)
            s_2 = storage_update(i,fd,1,storage,s_2)
            s_3 = storage_update(i,fd,2,storage,s_3)
            s_4 = storage_update(i,fd,3,storage,s_4)
            s_5 = storage_update(i,fd,4,storage,s_5)
            s_6 = storage_update(i,fd,5,storage,s_6)
            s_7 = storage_update(i,fd,6,storage,s_7)
            s_8 = storage_update(i,fd,7,storage,s_8)
            s_9 = storage_update(i,fd,8,storage,s_9)
            s_10 = storage_update(i,fd,9,storage,s_10)
            s_11 = storage_update(i,fd,10,storage,s_11)
            s_12 = storage_update(i,fd,11,storage,s_12)
            s_13 = storage_update(i,fd,12,storage,s_13)
            s_14 = storage_update(i,fd,13,storage,s_14)
            s_15 = storage_update(i,fd,14,storage,s_15)
            s_16 = storage_update(i,fd,15,storage,s_16)
            s_17 = storage_update(i,fd,16,storage,s_17)
            s_18 = storage_update(i,fd,17,storage,s_18)
            s_19 = storage_update(i,fd,18,storage,s_19)
            s_20 = storage_update(i,fd,19,storage,s_20)
            s_21 = storage_update(i,fd,20,storage,s_21)
            s_22 = storage_update(i,fd,21,storage,s_22)
            s_23= storage_update(i,fd,22,storage,s_23)
            s_24 = storage_update(i,fd,23,storage,s_24)
            s_25 = storage_update(i,fd,24,storage,s_25)
            s_26 = storage_update(i,fd,25,storage,s_26)
            s_27 = storage_update(i,fd,26,storage,s_27)
            s_28= storage_update(i,fd,27,storage,s_28)
            s_29 = storage_update(i,fd,28,storage,s_29)
            s_30 = storage_update(i,fd,29,storage,s_30)
            s_31 = storage_update(i,fd,30,storage,s_31)
            s_32 = storage_update(i,fd,31,storage,s_32)
            s_33 = storage_update(i,fd,32,storage,s_33)
            s_34 = storage_update(i,fd,33,storage,s_34)
            s_35 = storage_update(i,fd,34,storage,s_35)
            s_36 = storage_update(i,fd,35,storage,s_36)
            s_37 = storage_update(i,fd,36,storage,s_37)
            s_38 = storage_update(i,fd,37,storage,s_38)
            s_39 = storage_update(i,fd,38,storage,s_39)
            s_40 = storage_update(i,fd,39,storage,s_40)
            s_41 = storage_update(i,fd,40,storage,s_41)
            s_42 = storage_update(i,fd,41,storage,s_42)
            s_43 = storage_update(i,fd,42,storage,s_43)
            s_44 = storage_update(i,fd,43,storage,s_44)
            ...
            discharge[i,8,fd] = local.iloc[i,8]
            powerflow[i,8,fd] = min(f_max[8],discharge[i,8,fd]*0.001987)
            spill[i,8,fd] = max(0,discharge[i,8,fd]*.001987-f_max[8])
            storage[i,8,fd] = s_9
            ...
            pf_trans = powerflow[i,8,fd]*14.27641
            #Averagy hourly power output (MWh)
            generation[i,8,fd] = max(0,((1000*19.812*pf_trans*9.81*.86)/1000000)*24)
        
            #Upper Falls - run of river project
            discharge[i,23,fd] = local.iloc[i,23] + local.iloc[i,8]
            powerflow[i,23,fd] = min(f_max[23],discharge[i,23,fd]*.001987)
            spill[i,23,fd] = max(0,discharge[i,23,fd]*.001987-f_max[23])
            storage[i,23,fd] = s_24
            generation[i,23,fd] = max(0,((1000*19.812*powerflow[i,23,fd]*14.27641*9.81*.73)/1000000)*24)
        
            #Monroe Street - run of river project
            discharge[i,24,fd] = local.iloc[i,24] + local.iloc[i,23] + local.iloc[i,8]
            powerflow[i,24,fd] = min(f_max[24],discharge[i,24,fd]*.001987)
            spill[i,24,fd] = max(0,discharge[i,24,fd]*.001987-f_max[24])
            storage[i,24,fd] = s_25
            generation[i,24,fd] = max(0,((1000*23.1648*powerflow[i,24,fd]*14.27641*9.81*.82)/1000000)*24)
        
            #Nine Mile - run of river project
            discharge[i,25,fd] = local.iloc[i,25] + local.iloc[i,24] + local.iloc[i,23] + local.iloc[i,8]
            powerflow[i,25,fd] = min(f_max[25],discharge[i,25,fd]*.001987)
            spill[i,25,fd] = max(0,discharge[i,25,fd]*.001987-f_max[25])
            storage[i,25,fd] = s_26
            generation[i,25,fd] = max(0,((1000*20.42*powerflow[i,25,fd]*14.27641*9.81*.71)/1000000)*24)
        
            #Long Lake - run of river project
            discharge[i,26,fd] = local.iloc[i,26] + local.iloc[i,25] + local.iloc[i,24] + local.iloc[i,23] + local.iloc[i,8]
            powerflow[i,26,fd] = min(f_max[26],discharge[i,26,fd]*.001987)
            spill[i,26,fd] = max(0,discharge[i,26,fd]*.001987-f_max[26])
            storage[i,26,fd] = s_26
            generation[i,26,fd] = max(0,((1000*53.34*powerflow[i,26,fd]*14.26641*9.81*.76)/1000000)*24)
        
            #Litle Falls - run of river project
            discharge[i,27,fd] = local.iloc[i,27] + local.iloc[i,26] + local.iloc[i,25] + local.iloc[i,24] + local.iloc[i,23] + local.iloc[i,8]
            powerflow[i,27,fd] = min(f_max[27],discharge[i,27,fd]*.001987)
            spill[i,27,fd] = max(0,discharge[i,27,fd]*.001987-f_max[27])
            storage[i,27,fd] = s_27
            generation[i,27,fd] = max(0,((1000*19.812*powerflow[i,27,fd]*14.27641*9.81*.8)/1000000)*24)
        
            spokane_flow[i,fd] = local.iloc[i,8] + local.iloc[i,23] + local.iloc[i,24] + local.iloc[i,25] + local.iloc[i,26] + local.iloc[i,27]
            ...
            if i==0 and fd<1:
                MCD_sfore[i,fd]=min((local.iloc[i,0]-VPDR_MCD)*.001987+s_1,s_max[0])
            elif i==0 and fd>0:
            ...
            #Evacuation mode
                ...
                if overflow>0 and stor_avail<=0:
                    ...
                    #spilled.
                    powerflow[i,0,fd] = f_max[0]
                    spill[i,0,fd] = MCD_dg - f_max[0]
                    ...
                    powerflow[i,0,fd] = f_max[0]
                    spill[i,0,fd] = max(0,MCD_dg - f_max[0])
                else:
                    powerflow[i,0,fd] = MCD_dg
        
        
                s_1 = s_1 + local.iloc[i,0]*.001987 - MCD_dg
                discharge[i,0,fd] = MCD_dg*(1/.001987)  #Convert back to cfs
                storage[i,0,fd] = s_1
            ...
            #threshold).
                ...
                s_1 = s_1 + local.iloc[i,0]*.001987 - MCD_dg
                discharge[i,0,fd] = MCD_dg*(1/.001987)
                storage[i,0,fd] = s_1
            ...
            powerflow[i,15,fd] = min(RVC_dg,f_max[15])
            spill[i,15,fd] = max(0,RVC_dg - f_max[15])
            discharge[i,15,fd] = RVC_dg*(1/.001987)
            storage[i,15,fd] = s_16
            generation[i,15,fd] = ((1000*174.9552*powerflow[i,15,fd]*14.27641*9.81*.875)/1000000)*24
          ...
          #ARD's storage forecaste
              ARD_sfore[i,fd]=min((local.iloc[i,1]-VPDR_ARD)*.001987+s_2,s_max[1])      
            elif i<1 and fd >0:
              ARD_sfore[i,fd]=min(RVC_dg +(local.iloc[i,1]-VPDR_ARD)*.001987+ storage[i,1,fd-1],s_max[1])
            elif i>0 and fd<1:
              ARD_sfore[i,fd]=min(RVC_dg +(local.iloc[i,1]-VPDR_ARD)*.001987+ storage[i-1,1,fd],s_max[1])
            else:
              ARD_sfore[i,fd]=min(RVC_dg +(local.iloc[i,1]-VPDR_ARD)*.001987+ storage[i,1,fd-1],s_max[1])
        
            ARD_VRC = ARD_sfore[i,fd] #this is in kAF
            ...
            #convert project inflows ("local") to kAF.
                ...
                if overflow>0 and stor_avail<=0:
                    ...
                    #the same, and anything beyond hydraulic capacity of dam is
                    #spilled.
                    powerflow[i,1,fd] = f_max[1]
                    spill[i,1,fd] = ARD_dg - f_max[1]
                elif overflow>0 and stor_avail>0:
                    #If there is some storage available, total release must be revised down and additional water
                    #stored.
                    ARD_dg = max(ARD_dg - stor_avail,f_max[1])
                    powerflow[i,1,fd] = f_max[1]
                    spill[i,1,fd] = max(0,ARD_dg - f_max[1])
                else:
                    powerflow[i,1,fd] = ARD_dg
        
        
                s_2 = s_2 + RVC_dg + local.iloc[i,1]*.001987 - ARD_dg
                discharge[i,1,fd] = ARD_dg*(1/.001987)  #Convert back to cfs
                storage[i,1,fd] = s_2
            ...
            #threshold).
                ...
                s_2 = s_2 + RVC_dg + local.iloc[i,1]*.001987 - ARD_dg
                discharge[i,1,fd] = ARD_dg*(1/.001987)
                storage[i,1,fd] = s_2
        
        
        
            generation[i,1,fd] = ((1000*52.1208*powerflow[i,1,fd]*14.27641*9.81*.875)/1000000)*24
            ...
            #from Jan 1 to May 30 the value is all 100 cfs
              ...
              #DNC's storage forecaste
                  ...
                  DNC_sfore[i]=min((local.iloc[i,3]-VPDR_DNC)*.001987+ storage[i,3,fd-1],s_max[3])
               
                elif i > 0 and fd < 1: 
                  DNC_sfore[i]=min((local.iloc[i,3]-VPDR_DNC)*.001987+ storage[i-1,3,fd],s_max[3])
                    
                else:
                  DNC_sfore[i]=min((local.iloc[i,3]-VPDR_DNC)*.001987+ storage[i,3,fd-1],s_max[3])
                
                DNC_VRC= DNC_sfore[i,fd] #this is in kAF
                ...
                discharge[i,3,fd] = DNC_dg*(1/.001987)  #Convert back to cfs
                storage[i,3,fd] = s_4
            ...
            #threshold).
                ...
                s_4 = s_4 + local.iloc[i,3]*.001987 - DNC_dg
                discharge[i,3,fd] = DNC_dg*(1/.001987)
                storage[i,3,fd] = s_4
            ...
            if i==0 and  fd<1:
         #DNC's storage forecaste
                 LIB_sfore[i,fd]=min((local.iloc[i,2]-VPDR_LIB)*.001987+s_3,s_max[2])
            elif i>0 and fd<1:
                 LIB_sfore[i,fd]=min((local.iloc[i,2]-VPDR_LIB)*.001987+ storage[i-1,2,fd],s_max[2])
            else:
                 LIB_sfore[i,fd]=min((local.iloc[i,2]-VPDR_LIB)*.001987+ storage[i,2,fd-1],s_max[2])
                 
                
            LIB_VRC= LIB_sfore[i,fd] #this is in kAF
             #Identify target reservoir storage. A number of flood control curves
                #are publically available. They are based on forecasted inflows. Some interpolation is
                #necessary when the forecasted flows are in-between those specifiec by
                #USACE.
        ...
        #    FCDend_LIB = np.zeros((years,1))
            ...
            #Flow target
                ...
                if overflow>0 and stor_avail<=0:
                    ...
                    #the same, and anything beyond hydraulic capacity of dam is
                    #spilled.
                    powerflow[i,2,fd] = f_max[2]
                    spill[i,2,fd] = LIB_dg - f_max[2]
                elif overflow>0 and stor_avail>0:
                    #If there is some storage available, total release must be revised down and additional water
                    #stored.
                    LIB_dg = max(LIB_dg - stor_avail,f_max[2])
                    powerflow[i,2,fd] = f_max[2]
                    spill[i,2,fd] = max(0,LIB_dg - f_max[2])
                else:
                    powerflow[i,2,fd] = LIB_dg
        
        
                s_3 = max(0,s_3 + local.iloc[i,2]*.001987 - LIB_dg)
                discharge[i,2,fd] = LIB_dg*(1/.001987)  #Convert back to cfs
                storage[i,2,fd] = s_3
        ...
        #        VARQnew_LIB = np.zeros((sim_days,1))
                if year >1:
                    VAQR_LIB[i,fd]= LIB_FL
                    LIB_dg = max(max(4000*.001987, VAQR_LIB[i,fd]*.001987),s_3 + local.iloc[i,2]*.001987 - LIB_ORC[julian-1]+ VAQR_LIB[i,fd]*.001987*1.5)
                else:
                    LIB_dg = max(4000*.001987,s_3 + local.iloc[i,2]*.001987 - LIB_ORC[julian-1])
        
                VAQR_LIB[i,fd]= LIB_FL
                VAQRnew_LIB[i,fd]=VAQR_LIB[i,fd]  #+ ADJ_LIB(year)
                ...
                s_3 = max(0,s_3 + local.iloc[i,2]*.001987 - LIB_dg)
                discharge[i,2,fd] = LIB_dg*(1/.001987)
                storage[i,2,fd] = s_3
            ...
            LIB_tailwater = TailEq.iloc[2,1]*discharge[i,2,fd]**4 + TailEq.iloc[2,2]*discharge[i,2,fd]**3 + TailEq.iloc[2,3]*discharge[i,2,fd]**2 + TailEq.iloc[2,4]*discharge[i,2,fd] + TailEq.iloc[2,5]
            #Convert head to meters.
            #LIB_head = np.zeros((sim_days,1))
            LIB_head[i,fd] = (LIB_forebay - LIB_tailwater)*.3048
            generation[i,2,fd] = max(0,((1000*LIB_head[i,fd]*powerflow[i,2,fd]*14.27641*9.81*.865)/1000000)*24)
            ...
            powerflow[i,46,fd] = min(BFE_dg,f_max[46])
            spill[i,46,fd] = max(0,BFE_dg - f_max[46])
            s_47 = s_47 + LIB_dg + local.iloc[i,46]*.001987 - BFE_dg
            discharge[i,46,fd] = BFE_dg*(1/.001987)
            storage[i,46,fd] = s_47
            generation[i,46,fd] = ((1000*28.0416*powerflow[i,46,fd]*14.27641*9.81*.875)/1000000)*24
            ...
            powerflow[i,4,fd] = min(CLN_dg,f_max[4])
            spill[i,4,fd] = max(0,CLN_dg - f_max[4])
            s_5 = s_5 + BFE_dg + DNC_dg + local.iloc[i,4]*.001987 - CLN_dg
            discharge[i,4,fd] = CLN_dg*(1/.001987)
            storage[i,4,fd] = s_5
            generation[i,4,fd] = ((1000*16.1544*powerflow[i,4,fd]*14.27641*9.81*.875)/1000000)*24
            ...
            powerflow[i,17,fd] = min(BRI_dg,f_max[17])
            spill[i,17,fd] = max(0,BRI_dg - f_max[17])
            s_18 = s_18 + CLN_dg + local.iloc[i,17]*.001987 - BRI_dg
            discharge[i,17,fd] = BRI_dg*(1/.001987)
            storage[i,17,fd] = s_18
            generation[i,17,fd] = ((1000*28.0416*powerflow[i,17,fd]*14.27641*9.81*.875)/1000000)*24
          ...
          ########################################################################
            ...
            #convert project inflows ("local") to kAF. Note: there is a minimum flow release of 3500cfs associated with this dam.
                ...
                if overflow>0 and stor_avail<=0:
                    ...
                    #the same, and anything beyond hydraulic capacity of dam is
                    #spilled.
                    powerflow[i,5,fd] = f_max[5]
                    spill[i,5,fd] = HHO_dg - f_max[5]
                elif overflow>0 and stor_avail>0:
                    #If there is some storage available, total release must be revised down and additional water
                    #stored.
                    HHO_dg = max(HHO_dg - stor_avail,f_max[5])
                    powerflow[i,5,fd] = f_max[5]
                    spill[i,5,fd] = max(0,HHO_dg - f_max[5])
                else:
                    powerflow[i,5,fd] = HHO_dg
           
                s_6 = s_6 + local.iloc[i,5]*.001987 - HHO_dg
                discharge[i,5,fd] = HHO_dg*(1/.001987)  #Convert back to cfs
                storage[i,5,fd] = s_6
        ...
        #        VAQRnew_HHO = np.zeros((sim_days,1))
                if year >1:
                    VAQR_HHO[i,fd]= HHO_FL
                    VAQRnew_HHO[i,fd]=VAQR_HHO[i,fd]  #+ ADJ_LIB(year)
                    HHO_dg = max(max(0,3500-local.iloc[i,6])*.001987, s_6 + local.iloc[i,5]*.001987 - HHO_ORC[julian-1]+ VAQR_HHO[i,fd]*.001987*1.5)
                else:
                    HHO_dg = max(max(0,3500-local.iloc[i,6])*.001987, s_6 + local.iloc[i,5]*.001987 - HHO_ORC[julian-1])
                ...
                s_6 = s_6 + local.iloc[i,5]*.001987 - HHO_dg
                discharge[i,5,fd] = HHO_dg*(1/.001987)
                storage[i,5,fd] = s_6
            ...
            #What is tailwater elevation?
            HHO_tailwater = TailEq.iloc[5,1]*discharge[i,5,fd]**4 + TailEq.iloc[5,2]*discharge[i,5,fd]**3 + TailEq.iloc[5,3]*discharge[i,5,fd]**2 + TailEq.iloc[5,4]*discharge[i,5,fd] + TailEq.iloc[5,5]
            #Convert head to meters.
            #HHO_head = np.zeros((sim_days,1))
            HHO_head[i,fd] = (HHO_forebay - HHO_tailwater)*.3048
            generation[i,5,fd] = max(0,((1000*HHO_head[i,fd]* powerflow[i,5,fd] *14.27641*9.81*.84)/1000000)*24)
            ...
            if i > 0:
                KER_dg = max(0,discharge[i-1,5,fd]*.001987 + + CFM5L.iloc[i]*.001987+ local.iloc[i,6]*.001987)
                s_7 = s_7 + discharge[i-1,5,fd]*.001987 + local.iloc[i,6]*.001987 + CFM5L.iloc[i]*.001987 - KER_dg
            else:
            ...
            powerflow[i,6,fd] = min(KER_dg,f_max[6])
            spill[i,6,fd] = max(0,KER_dg - f_max[6])
            discharge[i,6,fd] = KER_dg*(1/.001987)
            storage[i,6,fd] = s_7
            #Convert storage to AF
            KER_stor = s_7*1000
            smax = s_max[6]*1000
            ...
            generation[i,6,fd] = ((1000*62.7888*powerflow[i,6,fd]*14.27641*9.81*.875)/1000000)*24
            ...
            powerflow[i,18,fd] = min(THF_dg,f_max[18])
            spill[i,18,fd] = max(0,THF_dg - f_max[18])
            discharge[i,18,fd] = THF_dg*(1/.001987)
            storage[i,18,fd] = s_19
            generation[i,18,fd] = ((1000*19.5072*powerflow[i,18,fd]*14.27641*9.81*.875)/1000000)*24
            ...
            if i > 0:
                NOX_dg = max(0,discharge[i-1,18,fd]*.001987 + local.iloc[i,13]*.001987)
                s_14 = s_14 + discharge[i-1,18,fd]*.001987 + local.iloc[i,13]*.001987 - NOX_dg
            else:
                NOX_dg = max(0,THF_dg + local.iloc[i,13]*.001987)
                s_14 = s_14 + THF_dg + local.iloc[i,13]*.001987 - NOX_dg
        
            powerflow[i,13,fd] = min(NOX_dg,f_max[13])
            spill[i,13,fd] = max(0,NOX_dg - f_max[13])
            discharge[i,13,fd] = NOX_dg*(1/.001987)
            storage[i,13,fd] = s_14
            #Convert storage to AF
            NOX_stor = s_14*1000
            smax = s_max[13]*1000
            #What is forebay elevation?
            ...
            generation[i,13,fd] = ((1000*54.5592*powerflow[i,13,fd]*14.27641*9.81*.875)/1000000)*24
            ...
            powerflow[i,19,fd] = min(CBG_dg,f_max[19])
            spill[i,19,fd] = max(0,CBG_dg - f_max[19])
            discharge[i,19,fd] = CBG_dg*(1/.001987)
            storage[i,19,fd] = s_19
            generation[i,19,fd] = ((1000*33.8328*powerflow[i,19,fd]*14.27641*9.81*.875)/1000000)*24
            ...
            powerflow[i,45,fd] = min(PSL_dg,f_max[45])
            spill[i,45,fd] = max(0,PSL_dg - f_max[45])
            discharge[i,45,fd] = PSL_dg*(1/.001987)
            storage[i,45,fd] = s_45
            generation[i,45,fd] = ((1000*84.1428*powerflow[i,45,fd]*14.27641*9.81*.875)/1000000)*24
        ...
        #    ALB_sfore = np.zeros((sim_days,1))
            if i==0 and fd<1:
        #DNC's storage forecaste
                ALB_sfore[i,fd]=min((local.iloc[i,7]-4000)*.001987+ PSL_dg + CBG_dg+s_8,s_max[7])
            elif i>0 and fd<1:
                ALB_sfore[i,fd]=min((local.iloc[i,7]-4000)*.001987+ PSL_dg + CBG_dg+ storage[i-1,7,fd],s_max[7])
            else:
                ALB_sfore[i,fd]=min((local.iloc[i,7]-4000)*.001987+ PSL_dg + CBG_dg+ storage[i,7,fd-1],s_max[7])
                
            
            
            
            ALB_VRC= ALB_sfore[i,fd]
            #ALB_ORC = np.zeros((365,1))
            if julian>=213 and julian <=365:
            ...
            if overflow>0 and stor_avail<=0:
                    ...
                    #the same, and anything beyond hydraulic capacity of dam is
                    #spilled.
                powerflow[i,7,fd] = f_max[7]
                spill[i,7,fd] = ALB_dg - f_max[7]
            elif overflow>0 and stor_avail>0:
                    #If there is some storage available, total release must be revised down and additional water
                    #stored.
                ALB_dg = max(ALB_dg - stor_avail,f_max[7])
                powerflow[i,7,fd] = f_max[7]
                spill[i,7,fd] = max(0,ALB_dg - f_max[7])
            else:
                powerflow[i,7,fd] = ALB_dg
        
        
            s_8 = s_8 + local.iloc[i,7]*.001987 + PSL_dg + CBG_dg - ALB_dg
            discharge[i,7,fd] = ALB_dg*(1/.001987)
            storage[i,7,fd] = s_8
            ...
            #What is tailwater elevation?
            ALB_tailwater = TailEq.iloc[7,1]*discharge[i,7,fd]**4 + TailEq.iloc[7,2]*discharge[i,7,fd]**3 + TailEq.iloc[7,3]*discharge[i,7,fd]**2 + TailEq.iloc[7,4]*discharge[i,7,fd] + TailEq.iloc[7,5]
            #Convert head to meters.
            ALB_head = (ALB_forebay - ALB_tailwater)*.3048
            generation[i,7,fd] = max(0,((1000*ALB_head*powerflow[i,7,fd]*14.27641*9.81*.71)/1000000)*24)
            ...
            powerflow[i,20,fd] = min(BOX_dg,f_max[20])
            spill[i,20,fd] = max(0,BOX_dg - f_max[20])
            discharge[i,20,fd] = BOX_dg*(1/.001987)
            storage[i,20,fd] = s_21
            generation[i,20,fd] = ((1000*14.0208*powerflow[i,20,fd]*14.27641*9.81*.875)/1000000)*24
            ...
            powerflow[i,21,fd] = min(BOU_dg,f_max[21])
            spill[i,21,fd] = max(0,BOU_dg - f_max[21])
            discharge[i,21,fd] = BOU_dg*(1/.001987)
            storage[i,21,fd] = s_22
            generation[i,21,fd] = ((1000*103.632*powerflow[i,21,fd]*14.27641*9.81*.875)/1000000)*24
            ...
            powerflow[i,16,fd] = min(SVN_dg,f_max[16])
            spill[i,16,fd] = max(0,SVN_dg - f_max[16])
            discharge[i,16,fd] = SVN_dg*(1/.001987)
            storage[i,16,fd] = s_17
            generation[i,16,fd] = ((1000*64.92*powerflow[i,16,fd]*14.27641*9.81*.875)/1000000)*24
            ...
            powerflow[i,22,fd] = min(WNT_dg,f_max[22])
            spill[i,22,fd] = max(0,WNT_dg - f_max[22])
            discharge[i,22,fd] = WNT_dg*(1/.001987)
            storage[i,22,fd] = s_23
            generation[i,22,fd] = ((1000*23.1648*powerflow[i,22,fd]*14.27641*9.81*.875)/1000000)*24
            ...
            if i<1 and fd<1:
                  #DNC's storage forecaste
                  GCL_sfore[i,fd]=min(local.iloc[i,9]*.001987 + spokane_flow[i,fd]*.001987 + discharge[i,1,fd]*.001987 + discharge[i,17,fd]*.001987 + discharge[i,22,fd]*.001987+s_10 - VPDR_GCL,s_max[9])
            elif i>=1 and fd<1:
                  GCL_sfore[i,fd]=min(local.iloc[i,9]*.001987 + spokane_flow[i-1,fd]*.001987 + discharge[i-1,1,fd]*.001987 + discharge[i-1,17,fd]*.001987 + discharge[i-1,22,fd]*.001987+ storage[i-1,9,fd] - VPDR_GCL,s_max[9])
            else:
                  GCL_sfore[i,fd]=min(local.iloc[i,9]*.001987 + spokane_flow[i,fd-1]*.001987 + discharge[i,1,fd-1]*.001987 + discharge[i,17,fd-1]*.001987 + discharge[i,22,fd-1]*.001987+ storage[i,9,fd-1] - VPDR_GCL,s_max[9])
    
            GCL_VRC[julian-1]= GCL_sfore[i,fd] #this is in kAF
        ...
        #    Storage_left = np.zeros((sim_days,1))
            Storage_left[i,fd]=GCL_fctarget-GCL_ORC[julian-1]
            ...
            #Evacuation mode
                ...
                #Grand Coulee.
                if i>0 and fd<1:
                    GCL_dg = max(30000*.001987, s_10 + local.iloc[i,9]*.001987 + spokane_flow[i-1,fd]*.001987 + discharge[i-1,1,fd]*.001987 + discharge[i-1,17,fd]*.001987 + discharge[i-1,22,fd]*.001987 -  GCL_ORC[julian-1])
                    ...
                    if overflow>0 and stor_avail<=0:
                        ...
                        #the same, and anything beyond hydraulic capacity of dam is
                        #spilled.
                        powerflow[i,9,fd] = f_max[9]
                        spill[i,9,fd] = GCL_dg - f_max[9]
                        ...
                        powerflow[i,9,fd] = f_max[9]
                        spill[i,9,fd] = max(0,GCL_dg - f_max[9])
                    else:
                        powerflow[i,9,fd] = GCL_dg
                        
                    s_10 = s_10 + local.iloc[i,9]*.001987 + spokane_flow[i-1,fd]*.001987 + discharge[i-1,1,fd]*.001987 + discharge[i-1,17,fd]*.001987 + discharge[i-1,22,fd]*.001987 - GCL_dg                    
    
                elif i>0 and fd>=1:
                    GCL_dg = max(30000*.001987, s_10 + local.iloc[i,9]*.001987 + spokane_flow[i,fd-1]*.001987 + discharge[i,1,fd-1]*.001987 + discharge[i,17,fd-1]*.001987 + discharge[i,22,fd-1]*.001987 -  GCL_ORC[julian-1])
                    ...
                    GCL_dg = max(30000*.001987, s_10 + local.iloc[i,9]*.001987 + spokane_flow[i,fd]*.001987 + discharge[i,1,fd]*.001987 + discharge[i,17,fd]*.001987 + discharge[i,22,fd]*.001987 -  GCL_ORC[julian-1])
                ...
                #If flows exceed dam's hydraulic capacity.
                    ...
                    s_10 = s_10 + local.iloc[i,9]*.001987 + spokane_flow[i,fd]*.001987 + discharge[i,1,fd]*.001987 + discharge[i,17,fd]*.001987 + discharge[i,22,fd]*.001987 - GCL_dg
        
        
                discharge[i,9,fd] = GCL_dg*(1/.001987)  #Convert back to cfs
                storage[i,9,fd] = s_10
            ...
            #threshold).
                ...
                #If flows exceed dam's hydraulic capacity.
                    ...
                    s_10 = s_10 + local.iloc[i,9]*.001987 + spokane_flow[i,fd-1]*.001987 + discharge[i,1,fd-1]*.001987 + discharge[i,17,fd-1]*.001987 + discharge[i,22,fd-1]*.001987 - GCL_dg
            ...
            #What is tailwater elevation?
            GCL_tailwater = TailEq.iloc[9,1]*discharge[i,9,fd]**4 + TailEq.iloc[9,2]*discharge[i,9,fd]**3 + TailEq.iloc[9,3]*discharge[i,9,fd]**2 + TailEq.iloc[9,4]*discharge[i,9,fd] + TailEq.iloc[9,5]
            #Convert head to meters.
            #GCL_head = np.zeros((sim_days,1))
            GCL_head[i,fd] = (GCL_forebay - GCL_tailwater)*.3048
            generation[i,9,fd] = ((1000*GCL_head[i,fd]*powerflow[i,9,fd]*14.27641*9.81*.9)/1000000)*24
            #heads = np.zeros((sim_days,10))
        #    Spillheads = np.zeros((sim_days,11))
            heads[i,0,fd]=GCL_head[i,fd]
            Spillheads[i,0,fd]=GCL_head[i,fd]
            #stor = np.zeros((sim_days,1))
            #inflow = stor = np.zeros((sim_days,1))
            stor[i,fd]=GCL_stor
            if i>0:
                inflow[i,fd]=(local.iloc[i,9]*.001987 + spokane_flow[i-1,fd]*.001987 + discharge[i-1,1,fd]*.001987 + discharge[i-1,17,fd]*.001987 + discharge[i-1,22,fd]*.001987)/0.001987
            ...
            powerflow[i,28,fd] = min(CHJ_dg,f_max[28])
            spill[i,28,fd] = max(0,CHJ_dg - f_max[28])
            discharge[i,28,fd] = CHJ_dg*(1/.001987)
            storage[i,28,fd] = s_29
            #Convert storage to AF
            CHJ_stor = s_29*1000
            #What is forebay elevation?
            ...
            #What is tailwater elevation?
            CHJ_tailwater = TailEq.iloc[28,1]*discharge[i,28,fd]**4 + TailEq.iloc[28,2]*discharge[i,28,fd]**3 + TailEq.iloc[28,3]*discharge[i,28,fd]**2 + TailEq.iloc[28,4]*discharge[i,28,fd] + TailEq.iloc[28,5]
            #Convert head to meters.
            #CHJ_head = np.zeros((sim_days,1))
            CHJ_head[i,fd] = max(0,(CHJ_forebay - CHJ_tailwater)*.3048)
            generation[i,28,fd] = max(0,((1000*CHJ_head[i,fd]*powerflow[i,28,fd]*14.27641*9.81*.84)/1000000)*24)
            heads[i,1,fd]=CHJ_head[i,fd]
            Spillheads[i,1,fd]=CHJ_head[i,fd]
            ...
            powerflow[i,29,fd] = min(WLL_dg,f_max[29])
            spill[i,29,fd] = max(0,WLL_dg - f_max[29])
            discharge[i,29,fd] = WLL_dg*(1/.001987)
            storage[i,29,fd] = s_30
            generation[i,29,fd] = ((1000*15.9142*powerflow[i,29,fd]*14.27641*9.81*.87)/1000000)*24
            ...
            powerflow[i,10,fd] = min(CHL_dg,f_max[10])
            spill[i,10,fd] = max(0,CHL_dg - f_max[10])
            discharge[i,10,fd] = CHL_dg*(1/.001987)
            storage[i,10,fd] = s_11
            generation[i,10,fd] = ((1000*121.92*powerflow[i,10,fd]*14.27641*9.81*.848)/1000000)*24
            ...
            powerflow[i,30,fd] = min(RRH_dg,f_max[30])
            spill[i,30,fd] = max(0,RRH_dg - f_max[30])
            discharge[i,30,fd] = RRH_dg*(1/.001987)
            storage[i,30,fd] = s_31
            generation[i,30,fd] = ((1000*24.3108*powerflow[i,30,fd]*14.27641*9.81*.873)/1000000)*24
            ...
            powerflow[i,31,fd] = min(RIS_dg,f_max[31])
            spill[i,31,fd] = max(0,RIS_dg - f_max[31])
            discharge[i,31,fd] = RIS_dg*(1/.001987)
            storage[i,31,fd] = s_32
            generation[i,31,fd] = ((1000*11.5640*powerflow[i,31,fd]*14.27641*9.81*.874)/1000000)*24
            ...
            powerflow[i,32,fd] = min(WPM_dg,f_max[32])
            spill[i,32,fd] = max(0,WPM_dg - f_max[32])
            discharge[i,32,fd] = WPM_dg*(1/.001987)
            storage[i,32,fd] = s_33
            generation[i,32,fd] = ((1000*19.2734*powerflow[i,32,fd]*14.27641*9.81*.873)/1000000)*24
            ...
            powerflow[i,33,fd] = min(PRD_dg,f_max[33])
            spill[i,33,fd] = max(0,PRD_dg - f_max[33])
            discharge[i,33,fd] = PRD_dg*(1/.001987)
            storage[i,33,fd] = s_34
            generation[i,33,fd] = ((1000*17.698*powerflow[i,33,fd]*14.27641*9.81*.873)/1000000)*24
            ...
            if i==0 and fd<1:
        #DNC's storage forecast
                BRN_sfore[i,fd]=min((local.iloc[i,11]-3500)*.001987+s_12,s_max[11])
            elif i>0 and fd<1:
                BRN_sfore[i,fd]=min((local.iloc[i,11]-3500)*.001987+ storage[i-1,11,fd],s_max[11])
            else:
                BRN_sfore[i,fd]=min((local.iloc[i,11]-3500)*.001987+ storage[i,11,fd-1],s_max[11])
                
            
            BRN_VRC= BRN_sfore[i,fd]
            #BRN_ORC = np.zeros((365,1))
            if julian>=213 and julian <=365:
            ...
            #Evacuation mode
                ...
                if overflow>0 and stor_avail<=0:
                    ...
                    #the same, and anything beyond hydraulic capacity of dam is
                    #spilled.
                    powerflow[i,11,fd] = f_max[11]
                    spill[i,11,fd] = BRN_dg - f_max[11]
                elif overflow>0 and stor_avail>0:
                    #If there is some storage available, total release must be revised down and additional water
                    #stored.
                    BRN_dg = max(BRN_dg - stor_avail,f_max[11])
                    powerflow[i,11,fd] = f_max[11]
                    spill[i,11,fd] = max(0,BRN_dg - f_max[11])
                else:
                    powerflow[i,11,fd] = BRN_dg
        
        
                s_12 = s_12 + local.iloc[i,11]*.001987 - BRN_dg
                discharge[i,11,fd] = BRN_dg*(1/.001987)  #Convert back to cfs
                storage[i,11,fd] = s_12
            ...
            #threshold).
                ...
                s_12 = s_12 + local.iloc[i,11]*.001987 - BRN_dg
                discharge[i,11,fd] = BRN_dg*(1/.001987)
                storage[i,11,fd] = s_12
            ...
            generation[i,11,fd] = ((1000*BRN_head*powerflow[i,11,fd]*14.27641*9.81*.87)/1000000)*24
            ...
            powerflow[i,34,fd] = min(OXB_dg,f_max[34])
            spill[i,34,fd] = max(0,OXB_dg - f_max[34])
            discharge[i,34,fd] = OXB_dg*(1/.001987)
            storage[i,34,fd] = s_35
            generation[i,34,fd] = ((1000*31.2674*powerflow[i,34,fd]*14.27641*9.81*.875)/1000000)*24
            ...
            powerflow[i,43,fd] = min(HLC_dg,f_max[43])
            spill[i,43,fd] = max(0,HLC_dg - f_max[43])
            discharge[i,43,fd] = HLC_dg*(1/.001987)
            storage[i,43,fd] = s_44
            generation[i,43,fd] = ((1000*53.62*powerflow[i,43,fd]*14.27641*9.81*.875)/1000000)*24
            ...
            if i==0 and fd<1:
              #DNC's storage forecaste
              DWR_sfore[i,fd]=min((local.iloc[i,12]-1600)*.001987+s_13,s_max[12])
            elif i>0 and fd<1:
              DWR_sfore[i,fd]=min((local.iloc[i,12]-1600)*.001987+ storage[i-1,12,fd],s_max[12])
            else:
              DWR_sfore[i,fd]=min((local.iloc[i,12]-1600)*.001987+ storage[i,12,fd-1],s_max[12])
                
            
            DWR_VRC= DWR_sfore[i,fd]
            ...
            #Evacuation mode
                ...
                if overflow>0 and stor_avail<=0:
                    ...
                    #the same, and anything beyond hydraulic capacity of dam is
                    #spilled.
                    powerflow[i,12,fd] = f_max[12]
                    spill[i,12,fd] = DWR_dg - f_max[12]
                elif overflow>0 and stor_avail>0:
                    #If there is some storage available, total release must be revised down and additional water
                    #stored.
                    DWR_dg = max(DWR_dg - stor_avail,f_max[12])
                    powerflow[i,12,fd] = f_max[12]
                    spill[i,12,fd] = max(0,DWR_dg - f_max[12])
                else:
                    powerflow[i,12,fd] = DWR_dg
        
        
                s_13 = s_13 + local.iloc[i,12]*.001987 - DWR_dg
                discharge[i,12,fd] = DWR_dg*(1/.001987)  #Convert back to cfs
                storage[i,12,fd] = s_13
            ...
            #threshold).
                ...
                s_13 = s_13 + local.iloc[i,12]*.001987 - DWR_dg
                discharge[i,12,fd] = DWR_dg*(1/.001987)
                storage[i,12,fd] = s_13
            ...
            #What is tailwater elevation?
            DWR_tailwater = TailEq.iloc[12,1]*discharge[i,12,fd]**4 + TailEq.iloc[12,2]*discharge[i,12,fd]**3 + TailEq.iloc[12,3]*discharge[i,12,fd]**2 + TailEq.iloc[12,4]*discharge[i,12,fd] + TailEq.iloc[12,5]
            #Convert head to meters.
            #DWR_head = np.zeros((sim_days,1))
            DWR_head[i,fd] = (DWR_forebay - DWR_tailwater)*.3048
            generation[i,12,fd] = ((1000*DWR_head[i,fd]*powerflow[i,12,fd]*14.27641*9.81*.83)/1000000)*24
            Spillheads[i,2,fd]=DWR_head[i,fd]
            ...
            powerflow[i,35,fd] = min(LWG_dg,f_max[35])
            spill[i,35,fd] = max(0,LWG_dg - f_max[35])
            discharge[i,35,fd] = LWG_dg*(1/.001987)  #Convert back to cfs
            storage[i,35,fd] = s_36
            #Convert storage to AF
            LWG_stor = s_36*1000
            #What is forebay elevation?
            ...
            #What is tailwater elevation?
            LWG_tailwater = TailEq.iloc[35,1]*discharge[i,35,fd]**4 + TailEq.iloc[35,2]*discharge[i,35,fd]**3 + TailEq.iloc[35,3]*discharge[i,35,fd]**2 + TailEq.iloc[35,4]*discharge[i,35,fd] + TailEq.iloc[35,5]
            #Convert head to meters.
            LWG_head[i,fd] = (LWG_forebay - LWG_tailwater)*.3048
            generation[i,35,fd] = ((1000*LWG_head[i,fd]*powerflow[i,35,fd]*14.27641*9.81*.86)/1000000)*24
            heads[i,2,fd]=LWG_head[i,fd]
            Spillheads[i,3,fd]=LWG_head[i,fd]
            ...
            powerflow[i,36,fd] = min(LGO_dg,f_max[36])
            spill[i,36,fd] = max(0,LGO_dg - f_max[36])
            discharge[i,36,fd] = LGO_dg*(1/.001987)  #Convert back to cfs
            storage[i,36,fd] = s_37
            #Convert storage to AF
            LGO_stor = s_37*1000
            #What is forebay elevation?
            ...
            #What is tailwater elevation?
            LGO_tailwater = TailEq.iloc[36,1]*discharge[i,36,fd]**4 + TailEq.iloc[36,2]*discharge[i,36,fd]**3 + TailEq.iloc[36,3]*discharge[i,36,fd]**2 + TailEq.iloc[36,4]*discharge[i,36,fd] + TailEq.iloc[36,5]
            #Convert head to meters.
            LGO_head[i,fd] = (LGO_forebay - LGO_tailwater)*.3048
            generation[i,36,fd] = ((1000*LGO_head[i,fd]*powerflow[i,36,fd]*14.27641*9.81*.86)/1000000)*24
            heads[i,3,fd]=LGO_head[i,fd]
            Spillheads[i,4,fd]=LGO_head[i,fd]
            ...
            powerflow[i,37,fd] = min(LMN_dg,f_max[37])
            spill[i,37,fd] = max(0,LMN_dg - f_max[37])
            discharge[i,37,fd] = LMN_dg*(1/.001987)  #Convert back to cfs
            storage[i,37,fd] = s_38
            #Convert storage to AF
            LMN_stor = s_38*1000
            #What is forebay elevation?
            ...
            #What is tailwater elevation?
            LMN_tailwater = TailEq.iloc[37,1]*discharge[i,37,fd]**4 + TailEq.iloc[37,2]*discharge[i,37,fd]**3 + TailEq.iloc[37,3]*discharge[i,37,fd]**2 + TailEq.iloc[37,4]*discharge[i,37,fd] + TailEq.iloc[37,5]
            #Convert head to meters.
            LMN_head[i,fd] = (LMN_forebay - LMN_tailwater)*.3048
            generation[i,37,fd] = max(0,((1000*LMN_head[i,fd]*powerflow[i,37,fd]*14.27641*9.81*.83)/1000000)*24)
            heads[i,4,fd]=LMN_head[i,fd]
            Spillheads[i,5,fd]=LMN_head[i,fd]
            ...
            powerflow[i,38,fd] = min(IHR_dg,f_max[38])
            spill[i,38,fd] = max(0,IHR_dg - f_max[38])
            discharge[i,38,fd] = IHR_dg*(1/.001987)  #Convert back to cfs
            storage[i,38,fd] = s_39
            #Convert storage to AF
            IHR_stor = s_39*1000
            #What is forebay elevation?
            ...
            #What is tailwater elevation?
            IHR_tailwater = TailEq.iloc[38,1]*discharge[i,38,fd]**4 + TailEq.iloc[38,2]*discharge[i,38,fd]**3 + TailEq.iloc[38,3]*discharge[i,38,fd]**2 + TailEq.iloc[38,4]*discharge[i,38,fd] + TailEq.iloc[38,5]
            #Convert head to meters.
            IHR_head[i,fd] = (IHR_forebay - IHR_tailwater)*.3048
            generation[i,38,fd] = ((1000*IHR_head[i,fd]*powerflow[i,38,fd]*14.27641*9.81*.83)/1000000)*24
            heads[i,5,fd]=IHR_head[i,fd]
            Spillheads[i,6,fd]=IHR_head[i,fd]
            ...
            powerflow[i,14,fd] = min(RBU_dg,f_max[14])
            spill[i,14,fd] = max(0,RBU_dg - f_max[14])
            discharge[i,14,fd] = RBU_dg*(1/.001987)  #Convert back to cfs
            storage[i,14,fd] = s_15
            generation[i,14,fd] = ((1000*134*powerflow[i,14,fd]*14.27641*9.81*.83)/1000000)*24
            ...
            powerflow[i,39,fd] = min(MCN_dg,f_max[39])
            spill[i,39,fd] = max(0,MCN_dg - f_max[39])
            discharge[i,39,fd] = MCN_dg*(1/.001987)  #Convert back to cfs
            storage[i,39,fd] = s_40
            #Convert storage to AF
            MCN_stor = s_40*1000
            #What is forebay elevation?
            ...
            #What is tailwater elevation?
            MCN_tailwater = TailEq.iloc[39,1]*discharge[i,39,fd]**4 + TailEq.iloc[39,2]*discharge[i,39,fd]**3 + TailEq.iloc[39,3]*discharge[i,39,fd]**2 + TailEq.iloc[39,4]*discharge[i,39,fd] + TailEq.iloc[39,5]
            #Convert head to meters.
            MCN_head[i,fd] = (MCN_forebay - MCN_tailwater)*.3048
            generation[i,39,fd] = ((1000*MCN_head[i,fd]*powerflow[i,39,fd]*14.27641*9.81*.76)/1000000)*24
            heads[i,6,fd]=MCN_head[i,fd]
            Spillheads[i,7,fd]=MCN_head[i,fd]
            ...
            powerflow[i,40,fd] = min(JDA_dg,f_max[40])
            spill[i,40,fd] = max(0,JDA_dg - f_max[40])
            discharge[i,40,fd] = JDA_dg*(1/.001987)  #Convert back to cfs
            storage[i,40,fd] = s_41
            #Convert storage to AF
            JDA_stor = s_41*1000
            #What is forebay elevation?
            ...
            #What is tailwater elevation?
            JDA_tailwater = TailEq.iloc[40,1]*discharge[i,40,fd]**4 + TailEq.iloc[40,2]*discharge[i,40,fd]**3 + TailEq.iloc[40,3]*discharge[i,40,fd]**2 + TailEq.iloc[40,4]*discharge[i,40,fd] + TailEq.iloc[40,5]
            #Convert head to meters.
            JDA_head[i,fd] = max(0,(JDA_forebay - JDA_tailwater)*.3048)
            generation[i,40,fd] = max(0,((1000*JDA_head[i,fd]*powerflow[i,40,fd]*14.27641*9.81*.87)/1000000)*24)
            heads[i,7,fd]=JDA_head[i,fd]
            Spillheads[i,8,fd]=JDA_head[i,fd]
            ...
            powerflow[i,41,fd] = min(TDA_dg,f_max[41])
            spill[i,41,fd] = max(0,TDA_dg - f_max[41])
            discharge[i,41,fd] = TDA_dg*(1/.001987)  #Convert back to cfs
            storage[i,41,fd] = s_42
            #Convert storage to AF
            TDA_stor = s_42*1000
            #What is forebay elevation?
            TDA_forebay = 13.891*np.log(TDA_stor) - 20.692
            #What is tailwater elevation?
            TDA_tailwater = TailEq.iloc[41,1]*discharge[i,41,fd]**4 + TailEq.iloc[41,2]*discharge[i,41,fd]**3 + TailEq.iloc[41,3]*discharge[i,41,fd]**2 + TailEq.iloc[41,4]*discharge[i,41,fd] + TailEq.iloc[41,5]
            #Convert head to meters.
            TDA_head = max(0,(TDA_forebay - TDA_tailwater)*.3048)
            generation[i,41,fd] = max(0,((1000*TDA_head*powerflow[i,41,fd]*14.27641*9.81*.86)/1000000)*24)
            heads[i,8,fd]=TDA_head
            Spillheads[i,9,fd]=TDA_head
            ...
            powerflow[i,42,fd] = min(BNN_dg,f_max[42])
            spill[i,42,fd] = max(0,BNN_dg - f_max[42])
            discharge[i,42,fd] = BNN_dg*(1/.001987)  #Convert back to cfs
            storage[i,42,fd] = s_43
            #Convert storage to AF
            BNN_stor = s_43*1000
            #What is forebay elevation?
            ...
            #What is tailwater elevation?
            BNN_tailwater = TailEq.iloc[42,1]*discharge[i,42,fd]**4 + TailEq.iloc[42,2]*discharge[i,42,fd]**3 + TailEq.iloc[42,3]*discharge[i,42,fd]**2 + TailEq.iloc[42,4]*discharge[i,42,fd] + TailEq.iloc[42,5]
            #Convert head to meters.
            BNN_head = (BNN_forebay - BNN_tailwater)*.3048
            generation[i,42,fd] = max(0,((1000*BNN_head*powerflow[i,42,fd]*14.27641*9.81*.92)/1000000)*24)
            heads[i,9,fd]=BNN_head
    ...
    #differences=np.zeros((len(Total_generation)-f_horizon+1,f_horizon))
    #for i in range(0,len(Total_generation)-f_horizon+1):
    #    differences[i,:] = (TG[i,:] - TG[i:i+f_horizon,0])/1000  
    #
    #month_ID = np.zeros(((no_years-2)*365,1))
    ...
    #
    #plt.savefig('FCRPS_perfect_foresight.png', dpi=2000)
        
    ##Load Willamette
    
    for fd in range(0,f_horizon):
       
        filename = 'Willamette/Output_HP%d.csv' % fd
        sim_Willamette = pd.read_csv(filename, header=0,index_col = 0)
        a=sim_Willamette.values
    ...
    Path_dams[:,25,:] = WILL_total
    
    #Add Willamette and Willamette missing to Total PNW dams
    Will_PNW=sim_W*(1+514/411)
    WPNW_reshape = np.reshape(Will_PNW,(len(WILL_total),1,f_horizon))
    Total_generation=np.column_stack((Total_generation, WPNW_reshape))
    ...
    Total_sum.to_csv('PNW_hydro/PNW_hydro_daily.csv') #to be used in the dispatch
