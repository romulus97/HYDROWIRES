...
model.h3_periods = RangeSet(49,72)
model.h4_periods = RangeSet(73,96)
model.h5_periods = RangeSet(97,120)
model.h6_periods = RangeSet(121,144)
model.h7_periods = RangeSet(145,168)
...
model.SimSCE_hydro_minflow = Param(model.forecast_days, model.SHF_periods, within=NonNegativeReals)
...
model.SimPGE_valley_hydro_minflow = Param(model.forecast_days, model.SHF_periods, within=NonNegativeReals)
...
def HydroC1(model,i):
    m1 = sum(model.mwh_1['PGEV_hydro',i] for i in model.hh_periods)
    m2 = sum(model.mwh_2['PGEV_hydro',i] for i in model.hh_periods)
    m3 = sum(model.mwh_3['PGEV_hydro',i] for i in model.hh_periods)
    return m1 + m2 + m3 <= model.HorizonPGE_valley_hydro
model.HydroConstraint1= Constraint(model.hh_periods,rule=HydroC1)

def HydroC2(model,i):
    m1 = sum(model.mwh_1['SCE_hydro',i] for i in model.hh_periods)
    m2 = sum(model.mwh_2['SCE_hydro',i] for i in model.hh_periods)
    m3 = sum(model.mwh_3['SCE_hydro',i] for i in model.hh_periods)
    return m1 + m2 + m3 <= model.HorizonSCE_hydro
model.HydroConstraint2= Constraint(model.hh_periods,rule=HydroC2)
...
def ImportsC3(model,i):
    m1 = sum(model.mwh_1['P66I',i] for i in model.h3_periods)
    m2 = sum(model.mwh_2['P66I',i] for i in model.h3_periods)
    m3 = sum(model.mwh_3['P66I',i] for i in model.h3_periods)
    return m1 + m2 + m3 <= model.HorizonPath66_imports[3]
model.ImportsConstraint3= Constraint(model.h3_periods,rule=ImportsC3)

def ImportsC4(model,i):
    m1 = sum(model.mwh_1['P66I',i] for i in model.h4_periods)
    m2 = sum(model.mwh_2['P66I',i] for i in model.h4_periods)
    m3 = sum(model.mwh_3['P66I',i] for i in model.h4_periods)
    return m1 + m2 + m3 <= model.HorizonPath66_imports[4]
model.ImportsConstraint4= Constraint(model.h4_periods,rule=ImportsC4)

def ImportsC5(model,i):
    m1 = sum(model.mwh_1['P66I',i] for i in model.h5_periods)
    m2 = sum(model.mwh_2['P66I',i] for i in model.h5_periods)
    m3 = sum(model.mwh_3['P66I',i] for i in model.h5_periods)
    return m1 + m2 + m3 <= model.HorizonPath66_imports[5]
model.ImportsConstraint5= Constraint(model.h5_periods,rule=ImportsC5)

def ImportsC6(model,i):
    m1 = sum(model.mwh_1['P66I',i] for i in model.h6_periods)
    m2 = sum(model.mwh_2['P66I',i] for i in model.h6_periods)
    m3 = sum(model.mwh_3['P66I',i] for i in model.h6_periods)
    return m1 + m2 + m3 <= model.HorizonPath66_imports[6]
model.ImportsConstraint6= Constraint(model.h6_periods,rule=ImportsC6)

def ImportsC7(model,i):
    m1 = sum(model.mwh_1['P66I',i] for i in model.h7_periods)
    m2 = sum(model.mwh_2['P66I',i] for i in model.h7_periods)
    m3 = sum(model.mwh_3['P66I',i] for i in model.h7_periods)
    return m1 + m2 + m3 <= model.HorizonPath66_imports[7]
model.ImportsConstraint7= Constraint(model.h7_periods,rule=ImportsC7)


def ImportsC1_46(model,i):
...
model.ImportsConstraint1_46= Constraint(model.h1_periods,rule=ImportsC1_46)

def ImportsC2_46(model,i):
...
model.ImportsConstraint2_46= Constraint(model.h2_periods,rule=ImportsC2_46)

def ImportsC3_46(model,i):
    m1 = sum(model.mwh_1['P46I_SCE',i] for i in model.h3_periods)
    m2 = sum(model.mwh_2['P46I_SCE',i] for i in model.h3_periods)
    m3 = sum(model.mwh_3['P46I_SCE',i] for i in model.h3_periods)
    return m1 + m2 + m3 <= model.HorizonPath46_SCE_imports[3]
model.ImportsConstraint3_46= Constraint(model.h3_periods,rule=ImportsC3_46)

def ImportsC4_46(model,i):
    m1 = sum(model.mwh_1['P46I_SCE',i] for i in model.h4_periods)
    m2 = sum(model.mwh_2['P46I_SCE',i] for i in model.h4_periods)
    m3 = sum(model.mwh_3['P46I_SCE',i] for i in model.h4_periods)
    return m1 + m2 + m3 <= model.HorizonPath46_SCE_imports[4]
model.ImportsConstraint4_46= Constraint(model.h4_periods,rule=ImportsC4_46)

def ImportsC5_46(model,i):
    m1 = sum(model.mwh_1['P46I_SCE',i] for i in model.h5_periods)
    m2 = sum(model.mwh_2['P46I_SCE',i] for i in model.h5_periods)
    m3 = sum(model.mwh_3['P46I_SCE',i] for i in model.h5_periods)
    return m1 + m2 + m3 <= model.HorizonPath46_SCE_imports[5]
model.ImportsConstraint5_46= Constraint(model.h5_periods,rule=ImportsC5_46)

def ImportsC6_46(model,i):
    m1 = sum(model.mwh_1['P46I_SCE',i] for i in model.h6_periods)
    m2 = sum(model.mwh_2['P46I_SCE',i] for i in model.h6_periods)
    m3 = sum(model.mwh_3['P46I_SCE',i] for i in model.h6_periods)
    return m1 + m2 + m3 <= model.HorizonPath46_SCE_imports[6]
model.ImportsConstraint6_46= Constraint(model.h6_periods,rule=ImportsC6_46)

def ImportsC7_46(model,i):
    m1 = sum(model.mwh_1['P46I_SCE',i] for i in model.h7_periods)
    m2 = sum(model.mwh_2['P46I_SCE',i] for i in model.h7_periods)
    m3 = sum(model.mwh_3['P46I_SCE',i] for i in model.h7_periods)
    return m1 + m2 + m3 <= model.HorizonPath46_SCE_imports[7]
model.ImportsConstraint7_46= Constraint(model.h7_periods,rule=ImportsC7_46)


def ImportsC1_61(model,i):
...
model.ImportsConstraint1_61= Constraint(model.h1_periods,rule=ImportsC1_61)

def ImportsC2_61(model,i):
...
model.ImportsConstraint2_61= Constraint(model.h2_periods,rule=ImportsC2_61)

def ImportsC3_61(model,i):
    m1 = sum(model.mwh_1['P61I',i] for i in model.h3_periods)
    m2 = sum(model.mwh_2['P61I',i] for i in model.h3_periods)
    m3 = sum(model.mwh_3['P61I',i] for i in model.h3_periods)
    return m1 + m2 + m3 <= model.HorizonPath61_imports[3]
model.ImportsConstraint3_61= Constraint(model.h3_periods,rule=ImportsC3_61)

def ImportsC4_61(model,i):
    m1 = sum(model.mwh_1['P61I',i] for i in model.h4_periods)
    m2 = sum(model.mwh_2['P61I',i] for i in model.h4_periods)
    m3 = sum(model.mwh_3['P61I',i] for i in model.h4_periods)
    return m1 + m2 + m3 <= model.HorizonPath61_imports[4]
model.ImportsConstraint4_61= Constraint(model.h4_periods,rule=ImportsC4_61)

def ImportsC5_61(model,i):
    m1 = sum(model.mwh_1['P61I',i] for i in model.h5_periods)
    m2 = sum(model.mwh_2['P61I',i] for i in model.h5_periods)
    m3 = sum(model.mwh_3['P61I',i] for i in model.h5_periods)
    return m1 + m2 + m3 <= model.HorizonPath61_imports[5]
model.ImportsConstraint5_61= Constraint(model.h5_periods,rule=ImportsC5_61)

def ImportsC6_61(model,i):
    m1 = sum(model.mwh_1['P61I',i] for i in model.h6_periods)
    m2 = sum(model.mwh_2['P61I',i] for i in model.h6_periods)
    m3 = sum(model.mwh_3['P61I',i] for i in model.h6_periods)
    return m1 + m2 + m3 <= model.HorizonPath61_imports[6]
model.ImportsConstraint6_61= Constraint(model.h6_periods,rule=ImportsC6_61)

def ImportsC7_61(model,i):
    m1 = sum(model.mwh_1['P61I',i] for i in model.h7_periods)
    m2 = sum(model.mwh_2['P61I',i] for i in model.h7_periods)
    m3 = sum(model.mwh_3['P61I',i] for i in model.h7_periods)
    return m1 + m2 + m3 <= model.HorizonPath61_imports[7]
model.ImportsConstraint7_61= Constraint(model.h7_periods,rule=ImportsC7_61)


def ImportsC1_42(model,i):
...
model.ImportsConstraint1_42= Constraint(model.h1_periods,rule=ImportsC1_42)

def ImportsC2_42(model,i):
...
model.ImportsConstraint2_42= Constraint(model.h2_periods,rule=ImportsC2_42)

def ImportsC3_42(model,i):
    m1 = sum(model.mwh_1['P42I',i] for i in model.h3_periods)
    m2 = sum(model.mwh_2['P42I',i] for i in model.h3_periods)
    m3 = sum(model.mwh_3['P42I',i] for i in model.h3_periods)
    return m1 + m2 + m3 <= model.HorizonPath42_imports[3]
model.ImportsConstraint3_42= Constraint(model.h3_periods,rule=ImportsC3_42)

def ImportsC4_42(model,i):
    m1 = sum(model.mwh_1['P42I',i] for i in model.h4_periods)
    m2 = sum(model.mwh_2['P42I',i] for i in model.h4_periods)
    m3 = sum(model.mwh_3['P42I',i] for i in model.h4_periods)
    return m1 + m2 + m3 <= model.HorizonPath42_imports[4]
model.ImportsConstraint4_42= Constraint(model.h4_periods,rule=ImportsC4_42)

def ImportsC5_42(model,i):
    m1 = sum(model.mwh_1['P42I',i] for i in model.h5_periods)
    m2 = sum(model.mwh_2['P42I',i] for i in model.h5_periods)
    m3 = sum(model.mwh_3['P42I',i] for i in model.h5_periods)
    return m1 + m2 + m3 <= model.HorizonPath42_imports[5]
model.ImportsConstraint5_42= Constraint(model.h5_periods,rule=ImportsC5_42)

def ImportsC6_42(model,i):
    m1 = sum(model.mwh_1['P42I',i] for i in model.h6_periods)
    m2 = sum(model.mwh_2['P42I',i] for i in model.h6_periods)
    m3 = sum(model.mwh_3['P42I',i] for i in model.h6_periods)
    return m1 + m2 + m3 <= model.HorizonPath42_imports[6]
model.ImportsConstraint6_42= Constraint(model.h6_periods,rule=ImportsC6_42)

def ImportsC7_42(model,i):
    m1 = sum(model.mwh_1['P42I',i] for i in model.h7_periods)
    m2 = sum(model.mwh_2['P42I',i] for i in model.h7_periods)
    m3 = sum(model.mwh_3['P42I',i] for i in model.h7_periods)
    return m1 + m2 + m3 <= model.HorizonPath42_imports[7]
model.ImportsConstraint7_42= Constraint(model.h7_periods,rule=ImportsC7_42)


def ImportsC1_24(model,i):
...
model.ImportsConstraint1_24= Constraint(model.h1_periods,rule=ImportsC1_24)

def ImportsC2_24(model,i):
...
model.ImportsConstraint2_24= Constraint(model.h2_periods,rule=ImportsC2_24)

def ImportsC3_24(model,i):
    m1 = sum(model.mwh_1['P24I',i] for i in model.h3_periods)
    m2 = sum(model.mwh_2['P24I',i] for i in model.h3_periods)
    m3 = sum(model.mwh_3['P24I',i] for i in model.h3_periods)
    return m1 + m2 + m3 <= model.HorizonPath24_imports[3]
model.ImportsConstraint3_24= Constraint(model.h3_periods,rule=ImportsC3_24)

def ImportsC4_24(model,i):
    m1 = sum(model.mwh_1['P24I',i] for i in model.h4_periods)
    m2 = sum(model.mwh_2['P24I',i] for i in model.h4_periods)
    m3 = sum(model.mwh_3['P24I',i] for i in model.h4_periods)
    return m1 + m2 + m3 <= model.HorizonPath24_imports[4]
model.ImportsConstraint4_24= Constraint(model.h4_periods,rule=ImportsC4_24)

def ImportsC5_24(model,i):
    m1 = sum(model.mwh_1['P24I',i] for i in model.h5_periods)
    m2 = sum(model.mwh_2['P24I',i] for i in model.h5_periods)
    m3 = sum(model.mwh_3['P24I',i] for i in model.h5_periods)
    return m1 + m2 + m3 <= model.HorizonPath24_imports[5]
model.ImportsConstraint5_24= Constraint(model.h5_periods,rule=ImportsC5_24)

def ImportsC6_24(model,i):
    m1 = sum(model.mwh_1['P24I',i] for i in model.h6_periods)
    m2 = sum(model.mwh_2['P24I',i] for i in model.h6_periods)
    m3 = sum(model.mwh_3['P24I',i] for i in model.h6_periods)
    return m1 + m2 + m3 <= model.HorizonPath24_imports[6]
model.ImportsConstraint6_24= Constraint(model.h6_periods,rule=ImportsC6_24)

def ImportsC7_24(model,i):
    m1 = sum(model.mwh_1['P24I',i] for i in model.h7_periods)
    m2 = sum(model.mwh_2['P24I',i] for i in model.h7_periods)
    m3 = sum(model.mwh_3['P24I',i] for i in model.h7_periods)
    return m1 + m2 + m3 <= model.HorizonPath24_imports[7]
model.ImportsConstraint7_24= Constraint(model.h7_periods,rule=ImportsC7_24)


def ImportsC1_45(model,i):
...
model.ImportsConstraint1_45= Constraint(model.h1_periods,rule=ImportsC1_45)

def ImportsC2_45(model,i):
...
model.ImportsConstraint2_45= Constraint(model.h2_periods,rule=ImportsC2_45)

def ImportsC3_45(model,i):
    m1 = sum(model.mwh_1['P45I',i] for i in model.h3_periods)
    m2 = sum(model.mwh_2['P45I',i] for i in model.h3_periods)
    m3 = sum(model.mwh_3['P45I',i] for i in model.h3_periods)
    return m1 + m2 + m3 <= model.HorizonPath45_imports[3]
model.ImportsConstraint3_45= Constraint(model.h3_periods,rule=ImportsC3_45)

def ImportsC4_45(model,i):
    m1 = sum(model.mwh_1['P45I',i] for i in model.h4_periods)
    m2 = sum(model.mwh_2['P45I',i] for i in model.h4_periods)
    m3 = sum(model.mwh_3['P45I',i] for i in model.h4_periods)
    return m1 + m2 + m3 <= model.HorizonPath45_imports[4]
model.ImportsConstraint4_45= Constraint(model.h4_periods,rule=ImportsC4_45)

def ImportsC5_45(model,i):
    m1 = sum(model.mwh_1['P45I',i] for i in model.h5_periods)
    m2 = sum(model.mwh_2['P45I',i] for i in model.h5_periods)
    m3 = sum(model.mwh_3['P45I',i] for i in model.h5_periods)
    return m1 + m2 + m3 <= model.HorizonPath45_imports[5]
model.ImportsConstraint5_45= Constraint(model.h5_periods,rule=ImportsC5_45)

def ImportsC6_45(model,i):
    m1 = sum(model.mwh_1['P45I',i] for i in model.h6_periods)
    m2 = sum(model.mwh_2['P45I',i] for i in model.h6_periods)
    m3 = sum(model.mwh_3['P45I',i] for i in model.h6_periods)
    return m1 + m2 + m3 <= model.HorizonPath45_imports[6]
model.ImportsConstraint6_45= Constraint(model.h6_periods,rule=ImportsC6_45)

def ImportsC7_45(model,i):
    m1 = sum(model.mwh_1['P45I',i] for i in model.h7_periods)
    m2 = sum(model.mwh_2['P45I',i] for i in model.h7_periods)
    m3 = sum(model.mwh_3['P45I',i] for i in model.h7_periods)
    return m1 + m2 + m3 <= model.HorizonPath45_imports[7]
model.ImportsConstraint7_45= Constraint(model.h7_periods,rule=ImportsC7_45)
