...
from PNW_dispatch_equal_pathflows import model as m1
from PNW_dispatchLP_equal_pathflows import model as m2
...
def sim(days):
    ...
    for day in range(1,days+1):
        ...
        if day < 2:
            ...
            pnw_shift = np.sum(pnwH_first) - instance.SimPNW_hydro['fd1',day-1]
            
            # update "pool" storage
            pnw_pool = pnw_pool - pnw_shift
            
            # correct for this difference in new amount to be scheduled, using "pooled" deficit/storage if there is any
            instance.HorizonPNW_hydro = np.max((0,pnw + pnw_pool))
            instance2.HorizonPNW_hydro = np.max((0,pnw + pnw_pool))
            
            p_tracker = np.append(p_tracker,pnw_pool)
            shift_tracker = np.append(shift_tracker,pnw_shift)
...
#        results = opt.solve(instance2)
    ...
    p_tracker_pd = pd.DataFrame(p_tracker)
    shift_tracker_pd = pd.DataFrame(shift_tracker)
    ...
    p_tracker_pd.to_csv('p_tracker.csv')
    shift_tracker_pd.to_csv('shift_tracker.csv')
